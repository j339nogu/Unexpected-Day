...
const unexpectedMissions = {
  short: [
    // 💡 テストしやすくするためにメロンソーダミッションを追加しています！
    { title: "至急、コンビニでメロンソーダを調達して撮影しろ", duration: 15, slots: ALL },
...
// --- 📷 カメラオーバーレイコンポーネント ---
const CameraOverlay = ({ onClose, onSuccess }) => {
  const videoRef = useRef(null);
  const canvasRef = useRef(null);
  const [isAnalyzing, setIsAnalyzing] = useState(false);
  const [message, setMessage] = useState('メロンソーダを画面の中央に捉えてください');
  ...
  const handleCapture = () => {
    ...
    for (let i = 0; i < data.length; i += 4) {
      ...
      // メロンソーダの「鮮やかな緑色」の条件
      // 緑の成分が赤・青より高く、一定の明るさがあること
      if (g > 100 && g > r * 1.5 && g > b * 1.2) {
        greenPixelCount++;
      }
    }
    ...
    setTimeout(() => {
      // 画面の5%以上がメロンソーダ色なら合格！
      if (greenRatio > 0.05) {
        alert(`メロンソーダを検出しました！（緑色率: ${Math.round(greenRatio * 100)}%）ミッションクリア！`);
        onSuccess();
      } else {
        setMessage('メロンソーダの色が足りません！観葉植物を撮っていませんか？');
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black flex flex-col items-center justify-center">
      <div className="absolute top-8 text-white font-bold text-center z-10 w-full px-4 drop-shadow-md">
        {message}
      </div>
      <video 
        ref={videoRef} 
      ...
      <div className="absolute bottom-12 flex flex-col gap-4 items-center w-full px-8">
        ...
        <button 
          onClick={onClose}
          className="text-white font-bold bg-gray-800 px-6 py-2 rounded-full opacity-80"
        >
          キャンセル
        </button>
      </div>
...
// --- イベントカードコンポーネント ---
const EventCard = ({ item, onDelete, onClick, onClear }) => {
  const isUnexpected = item.type === 'unexpected';
  const isCleared = item.isCleared; // クリア済みフラグ

  return (
    <div
      onClick={onClick}
      className={`
        relative w-full px-4 py-3 rounded-md border-2 mb-2 cursor-pointer transition-all hover:scale-[1.01]
    ...
    >
      ...
      {/* 予想外かつ未クリアの場合のみ、証拠撮影ボタンを表示 */}
      {isUnexpected && !isCleared && (
        <button
          onClick={(e) => { e.stopPropagation(); onClear(item.id); }}
          className="absolute bottom-3 right-4 text-xs font-bold bg-white text-[#C63527] px-3 py-1 rounded shadow hover:bg-gray-100"
        >
          📷 証拠提出
        </button>
      )}
      ...
      {/* クリア済みなら打ち消し線を入れる */}
      <div className={`font-bold text-base sm:text-lg leading-tight mb-1 pr-6 break-words whitespace-pre-wrap ${isCleared ? 'line-through' : ''}`}>
        {item.title}
      </div>
...
export default function App() {
  ...
  const [schedule, setSchedule] = useState([
    ...
    { id: 2, startTime: '13:00', endTime: '14:30', title: 'アプリ開発', type: 'normal' },
  ]);
  ...
  const saveEdit = (id) => {
    setSchedule(schedule.map(item => 
      item.id === id ? { ...item, title: editTitle, startTime: editStartTime, endTime: editEndTime } : item
    ));
    setEditingId(null);
  };
  ...
  const handleClearMission = (id) => {
    setSchedule(schedule.map(item => 
      item.id === id ? { ...item, isCleared: true } : item
    ));
    setCameraTargetId(null); // カメラを閉じる
  };
  ...
  const triggerUnexpected = () => {
    ...
    for (let i = 0; i < sorted.length - 1; i++) {
      ...
      if (startNext - endCurrent >= 15) {
        gaps.push({ start: endCurrent, end: startNext, duration: startNext - endCurrent });
      }
    }
    ...
    <>
      ...
      {cameraTargetId && (
        <CameraOverlay 
          ...
          onSuccess={() => handleClearMission(cameraTargetId)} 
        />
      )}
        ...
        <div className="w-full max-w-sm relative">
          ...
          <form onSubmit={addNormalEvent} className="mb-6 sm:mb-8 p-3 sm:p-4 border-2 border-dashed border-[#C63527]/50 rounded-md">
            ...
            <textarea
              ...
              rows={1} value={newTitle} onChange={(e) => { setNewTitle(e.target.value); handleTextareaResize(e); }}
            />
            ...
            {sortedDisplaySchedule.map((item) => (
              editingId === item.id ? (
                ...
                <EventCard
                  ...
                  onClear={(id) => setCameraTargetId(id)} // 📷ボタンでカメラIDをセット
                />
              )
            ))}
          </div>
